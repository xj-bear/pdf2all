...
const TOOLS = [
    {
        name: "pdf_to_docx",
        description: "[PDF2All] Convert PDF to DOCX (Word) format. Preserves layout, tables, and images. Supports files up to 100MB. Returns error for encrypted or corrupted PDFs.",
        inputSchema: {
            type: "object",
            properties: {
                pdf_path: {
    ...
    {
        name: "pdf_to_excel",
        description: "[PDF2All] Extract tables from PDF and save as Excel (.xlsx) file. Each table becomes a separate sheet. Supports OCR for image-based tables (requires Tesseract/RapidOCR). Works with both native PDF tables and scanned documents.",
        inputSchema: {
            type: "object",
            properties: {
                pdf_path: {
    ...
    {
        name: "pdf_to_ppt",
        description: "[PDF2All] Convert PDF to PowerPoint (.pptx) format. Each page becomes a slide with the page rendered as an image at 150 DPI. Ideal for presentations and slide decks.",
        inputSchema: {
            type: "object",
            properties: {
                pdf_path: {
    ...
    {
        name: "pdf_to_jpg",
        description: "[PDF2All] Convert PDF pages to JPG images. Creates one image per page at web-standard resolution (72 DPI). Output files are named with page numbers (e.g., filename_page_1.jpg).",
        inputSchema: {
            type: "object",
            properties: {
                pdf_path: {
...
async function executePythonConverter(action, pdfPath, outputPath, fastMode, dpi, quality, pages, useOcr) {
 ...
 */
async function prepareInputFile(pdfPath, pdfUrl, pdfBase64) {
    if (pdfPath) {
 ...
 */
async function handleOutput(result, inputTypeIsTemp) {
    ...
    if (result.output_paths) {
        const paths = result.output_paths;
 ...
 */
function createServer(baseUrl, publicDir) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        // Extract arguments
        const pdfPathArg = args?.pdf_path;
        const pdfUrl = args?.pdf_url;
        const pdfBase64 = args?.pdf_base64;
        // ... other args extraction
        const outputPath = args?.output_path;
        const fastMode = args?.fast_mode;
        const dpi = args?.dpi;
        const quality = args?.quality;
        const pages = args?.pages;
        const useOcr = args?.use_ocr;
        ...
        try {
            ...
            if (result.output_paths)
                console.error("DEBUG: output_paths length:", result.output_paths.length);
